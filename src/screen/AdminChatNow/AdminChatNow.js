...
//custom components
import AdminChatNowHeader from './AdminChatNowHeader';
...
const AdminChatNow = ({ route, userToken, navigation }) => {
  ...
  const chatRenderFunction = ({ item }) => (
    <View
      ...
      style={{
        marginVertical: 10,
        alignItems: userId == item.userId ? 'flex-start' : 'flex-end',
      }}>
      <View
        style={{
          backgroundColor:
            userId == item.userId ? Colors.WHITE : Colors.LITEGREEN,
          borderRadius: 10,
          padding: 10,
          width: '70%',
        }}>
        ...
        {item.message ? (
          <Hyperlink
            ...
            <Text
              style={{
                color: userId == item.userId ? Colors.BLACK : Colors.WHITE,
  ...
  return (
    <KeyboardAvoidingView
      ...
      <View style={styles.container}>
        <AdminChatNowHeader headerName={name} ImageUrl={ImageUrl} />
        <Image
        ...
        <View style={styles.mainView}>
          {MessagesData.length > 0 ? (
            <FlatList
              inverted
              ref={flatListRef}
              // onContentSizeChange={() => flatListRef.current.scrollToStart({ animated: false })}
              // onLayout={() => flatListRef.current.scrollToStart({ animated: false })}
...
export default connect(mapStateToProps, null)(AdminChatNow);
